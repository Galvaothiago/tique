    ...
    <section className="w-full h-60 bg-green-300">
        ...
        <div className="flex items-center justify-center gap-3 mt-4">
          {showMessage ? (
            ...
            numbersBet.map((number, i) => (
              <span
                key={`${i}-${number}`}
                className="grid place-items-center w-12 h-12 font-medium bg-green-100 rounded-full text-xl"
              >
                {number}
              </span>
            ))
      ...
      {!changeButtons ? (
        <div className="flex items-end justify-center gap-1 pb-1 w-full h-36">
          <button
            className="w-44 h-14 text-xs bg-green-100 text-center uppercase cursor-pointer"
            onClick={createBetAutomatically}
          >
        ...
        <div className="flex items-end justify-center gap-1 pb-1 w-full h-36">
          <button
            className="grid place-items-center w-44 h-14  bg-green-100 text-4xl text-red-400 cursor-pointer"
            onClick={removeBet}
          >
            <IoClose />
          </button>
          <button
            className="grid place-items-center w-44 h-14 bg-green-100 text-3xl text-green-400 cursor-pointer"
            onClick={addBet}
          >
            <AiOutlineCheck />
          </button>
